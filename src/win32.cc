...
int main(int argc, char* argv[])
{
...
#if __APPLE__ && TARGET_OS_OSX
    {
        ...
        if (home) {
            char gamePath[1024];
            snprintf(gamePath, sizeof(gamePath), "%s/Games/Fallout 2 CE", home);
            mkdir(gamePath, 0755);
            chdir(gamePath);
            fprintf(stderr, "[main] macOS game path: %s\n", gamePath);
        } else {
            fprintf(stderr, "[main] Could not determine HOME directory\n");
        }
    }
#endif
...
int main(int argc, char* argv[])
{
    ...
    // TODO: CopyDirectoryRecursive is defined but never called. If game data
    // is meant to be installed from D: to E: on first run, add the call here,
    // e.g.: CopyDirectoryRecursive("D:\\DATA", "E:\\UDATA\\Fallout2\\DATA", CopyDirectoryRecursive)
    // For now, game data is read directly from D:.
