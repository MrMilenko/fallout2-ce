...
#ifdef NXDK
// Converts a string to a double without locale support.
// NXDK's strtod asserts on certain inputs, so we use a custom implementation.
static double simpleStrtod(const char* str, char** endptr)
{
    const char* p = str;
    double result = 0.0;
    bool negative = false;
